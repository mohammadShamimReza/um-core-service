...
const updateStudentMark = async (payload: any) => {
  ...
  const result = StudentEnrolledCourseMarkUtils.getGradeFromMark(marks);
...
const updataFinalMarks = async (payload: any) => {
  ...
  const studenEnrolledCourseMarks =
  ...
  const midTermMarks =
    studenEnrolledCourseMarks.find(item => item.examType === ExamType.MIDTERM)
      ?.marks || 0;

  const finalTermMarks =
    studenEnrolledCourseMarks.find(item => item.examType === ExamType.FINAL)
      ?.marks || 0;

  const totalFinalMarks =
    Math.ceil(midTermMarks * 0.4) + Math.ceil(finalTermMarks * 0.4);

  const result =
    StudentEnrolledCourseMarkUtils.getGradeFromMark(totalFinalMarks);
