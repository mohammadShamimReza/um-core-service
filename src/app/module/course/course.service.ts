...
const insertIntoDB = async (data: ICourseCreateData): Promise<any> => {
  ...
  const newCourse = await prisma.$transaction(async transactionClient => {
    ...
    if (preRequisiteCourses && preRequisiteCourses.length > 0) {
      await asyncForEach(
        preRequisiteCourses,
        async (preRequisiteCourse: IPrerequisitesCourseRequest, i: any) => {
          console.log(i);
  ...
  await prisma.$transaction(async transactionClient => {
    ...
    if (preRequisiteCourses && preRequisiteCourses.length > 0) {
      const deletedPrerequisites = preRequisiteCourses.filter(
        coursePrerequisite =>
          coursePrerequisite.courseId && coursePrerequisite.idDeleted
      );
      ...
      await asyncForEach(
        ...
        async (deletedPrerequisite: IPrerequisitesCourseRequest) => {
      ...
      await asyncForEach(
        newPrerequisites,
        async (newPrerequisite: IPrerequisitesCourseRequest) => {
          await transactionClient.courseToPrerequisite.create({
            data: {
              courseId: id,
